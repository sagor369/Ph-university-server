...
const updateCourseInToDb = async (id: string, payload: TCourse) => {
  ...
  try{
   session.startTransaction()
    const updatesomeData = await CourseModel.findByIdAndUpdate(id, courseRemainingData, {
      new: true,
      runValidators:true,
      session
    });
    ...
    if (preRequisteCourses && preRequisteCourses.length > 0) {
      ...
      const deletecourse = await CourseModel.findByIdAndUpdate(id, {
        $pull: { preRequisteCourses: { course: { $in: deletedPreReCourse } } },
      },
      {
      new: true,
      runValidators:true,
    ...
    throw new AppError(httpStatus.BAD_REQUEST, "course data do not update ")
     ...
     const addNewCourse =  await CourseModel.findByIdAndUpdate(id, {
    ...
    const result = await CourseModel.findById(id).populate("preRequisteCourses.course")
...
const courseAssignInToDb = async(id:string, payload:Partial< TCourseFaculty>) =>{
